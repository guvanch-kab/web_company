...
if (isset($_POST['selected_date'])) {
   ...
   // $check_query = "SELECT * FROM controlgate1 WHERE DateEntry='$today' AND DeviceIPAddress IN ('10.1.10.210', '10.1.10.211')";
    $check_query = "SELECT * FROM controlgate1 WHERE DateEntry='$today' AND DeviceIPAddress='10.1.10.210'";
...
//$query = "SELECT * FROM AttendanceRecordInfo WHERE PersonName IS NOT NULL AND DATE(FROM_UNIXTIME(AttendanceDateTime / 1000)) = '$today' AND (DeviceIPAddress = '10.1.10.210'  OR  DeviceIPAddress = '10.1.10.211')";
$query = "SELECT * FROM AttendanceRecordInfo WHERE PersonName IS NOT NULL AND DATE(FROM_UNIXTIME(AttendanceDateTime / 1000)) = '$today' AND DeviceIPAddress = '10.1.10.210' ";
$netije = mysqli_query($conn, $query);

if (mysqli_num_rows($netije) > 0) {
    $net = $netije;
} 
else if (mysqli_num_rows($check) > 0) { // Eğer controlgate1'den veri varsa, net değişkenine ata
    $net = $check;
}
   ...
   // $location_add = array();
    ...
    while ($row = $net->fetch_assoc()) {
       ...
       // $location_add[] = $row['DeviceName'];
        ...
        // Duplicate kontrolü
       // if ($deviceip == '10.1.10.210' || $deviceip == '10.1.10.211') {
            if ($deviceip == '10.1.10.210') {   
            $check_query = "SELECT * FROM controlgate1 WHERE PersonID='$personid' AND AttendanceDateTime='$change_date' AND DeviceIPAddress='$deviceip'";
            $check_result = mysqli_query($connect, $check_query);
        
            if (mysqli_num_rows($check_result) > 0) {
                // Duplicate varsa güncelle
                $sqlupd = "UPDATE controlgate1 SET DateEntry='$dateentry', TimeEntry='$timeentry', PersonName='$personname' WHERE PersonID='$personid' AND AttendanceDateTime='$change_date' AND DeviceIPAddress='$deviceip'";
...
<div class="col-md-12" id="netije">       
    ...
    <div class="row" style="padding:20px 18px;">
        ...
        <div class="col-md-2 senetakvimi">
            <input type="date" id="getdate" max="<?php echo date('Y-m-d'); ?>" class="form-control">
        </div>

        <div class="col-md-10" style="font-weight: 600; color:#4a4a4a; margin-top:20px" id="isgar_sany"><b>Ähli Gapy Girişleri </b>
